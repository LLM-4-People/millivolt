...
# (code "token_expired") without an upstream send. Adopt access_token and any
# returned refresh_token from the error body, retain the old refresh token
# when no replacement is returned, then retry. Model discovery refreshes
# transparently without returning tokens. If the refresh token is
